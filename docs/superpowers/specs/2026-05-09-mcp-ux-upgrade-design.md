...
**Where**: `persona_mcp_store/cli/build.py` 끝에 추가. enrich → store.parquet 작성 직후, store를 한 번 scan해서 axes별 value-counts + schema + n_personas 계산해 sidecar 작성.
...
2. `python -m persona_mcp_store.mcp_server` 실행 후 MCP 클라이언트가 `list_resources` 호출 시 `personas://catalog` 항목 노출.
